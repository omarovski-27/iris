...
The pill itself lives in `crates/iris-overlay` (`OverlayHandle`), which *can*
hold and draw the live transcript while it is on screen, behind an opt-in that
is off by default — see that crate's `README.md` for the contract. App wiring
is `iris-app::pill::OverlayPill` (a `PillSink`); see
`crates/iris-app/README.md`.

This matters for perceived speed beyond the measured numbers: the pill is on
screen for the whole utterance (waveform and elapsed time, and the words as
they arrive when live text is opted in), so key-release is a state change on
something already visible rather than a surface appearing from nothing, which
reads as faster than it is.
